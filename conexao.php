...
// Função responsável inseir uma pessoa no meu banco de dados
    function inserirPessoa() {
        $banco = abrirBanco();
        $sql = "INSERT INTO pessoa(nome, nascimento, endereco, telefone,instagram) 
        VALUES ('{$_POST["nome"]}','{$_POST["nascimento"]}','{$_POST["endereco"]}','{$_POST["telefone"]}','{$_POST["instagram"]}')";
        $banco->query($sql);
        $banco->close();
        voltarIndex();
    }
...
// Função responsável editar uma pessoa no meu banco de dados
    function alterarPessoa() {
        $banco = abrirBanco();
        $sql = "UPDATE pessoa SET nome='{$_POST["nome"]}',nascimento='{$_POST["nascimento"]}',endereco='{$_POST["endereco"]}',telefone='{$_POST["telefone"]}', instagram='{$_POST["instagram"]}' WHERE id='{$_POST["id"]}'";
        $banco->query($sql);
        $banco->close();
        voltarIndex();
    }
